...
vector<couple> Dijkstra(int startVertex, int totalVertex) // here totalvertex also work as 
{                                                        //  destination vertex
    ...
    vector <couple> shortestPath;

    int minPath = pathVertex.top();
    pathVertex.pop();

    shortestPath.push_back(make_pair(distanceFromSource[totalVertex], minPath));

    minPath = pathVertex.top();
    pathVertex.pop();

    shortestPath.push_back(make_pair(lastTime, minPath));
...
int shop(int n, int k, vector<string> centers, vector<vector<int>> roads) 
{
    ...
    couple firstMin = shortestPath[0];
    couple secondMin = shortestPath[1];

    if(k <= firstMin.second)
    {
        return firstMin.first;
    }
    
    return secondMin.second;
}
